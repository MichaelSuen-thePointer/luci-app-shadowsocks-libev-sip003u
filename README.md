...
The OpenWrt 25.12 port is maintained on `main`. The
`openwrt-25.12-port` branch currently points to the same baseline commit.
...
`MichaelSuen-thePointer/shadowsocks-c` `feature/sip003u` commit. This repository changes the
...
- changes the source from the obsolete release tarball to the SIP003U Git
  ...
  `49d2bf68b509f4b5a1c28992828212e5633bd390`;
- sets package version `3.3.6_p3-r1` and source date `2026-09-23`;
- removes `PKG_FIXUP:=autoreconf`, enables `CMAKE_INSTALL`, and includes
  `cmake.mk`;
- replaces Autotools arguments with system dependency mode, shared library
  linkage, executable-only output, and `BUILD_TESTING=OFF` CMake options;
- replaces the obsolete `libev` dependency with `libuv` and adds `libcares`
  to `ss-local`, which now uses asynchronous DNS;
...
The pinned SIP003U source now uses CMake, libuv, Mbed TLS 3, and native PCRE2.
...
Run the all-package wrapper from the `main` checkout:
...
shadowsocks-libev-config-3.3.6_p3-r1.apk
shadowsocks-libev-ss-local-3.3.6_p3-r1.apk
shadowsocks-libev-ss-redir-3.3.6_p3-r1.apk
shadowsocks-libev-ss-rules-3.3.6_p3-r1.apk
shadowsocks-libev-ss-server-3.3.6_p3-r1.apk
shadowsocks-libev-ss-tunnel-3.3.6_p3-r1.apk
```
...
The intended direct dependencies are available from the 25.12.5 SDK's pinned
base, packages, and LuCI feeds. Recheck the emitted APK metadata after each
build:

```text
libc, luci-base, libcares, libuv1, libmbedtls21, libpcre2, libpthread,
...
| `shadowsocks-libev-ss-local` | `libcares`, `libuv1`, `libmbedtls21`, `libpcre2`, `libpthread`, `libsodium`, config |
| `shadowsocks-libev-ss-redir` | `libuv1`, `libmbedtls21`, `libpthread`, `libsodium`, config |
| `shadowsocks-libev-ss-rules` | `firewall4`, `ip-tiny`, `kmod-nft-tproxy`, `resolveip`, `ss-redir`, config, `ucode`, `ucode-mod-fs` |
| `shadowsocks-libev-ss-server` | `libcares`, `libuv1`, `libmbedtls21`, `libpcre2`, `libpthread`, `libsodium`, config |
| `shadowsocks-libev-ss-tunnel` | `libuv1`, `libmbedtls21`, `libpthread`, `libsodium`, config |
...
- SIP003U runtime source:
  `https://github.com/MichaelSuen-thePointer/shadowsocks-c.git`
- SIP003U source commit:
  `49d2bf68b509f4b5a1c28992828212e5633bd390`

The source commit was verified as the remote `feature/sip003u` head on 2026-09-23.
The build pins the commit rather than the moving branch name for repeatability.

The files taken from the two legacy commits are stored under
`vendor/openwrt-23.05/`; see its `README.md` for provenance.

The custom versions use `_p3`, which is valid in OpenWrt 25.12's APK version
